...
class _AddHouseImagesState extends ConsumerState<AddHouseImages> {
  ...
  Widget build(
    ...
    bool isButtonEnabled =
        ref.watch(imagesHouseProvider.select((state) => state.isButtonEnabled));
    List<XFile> imageFileList =
        ref.watch(imagesHouseProvider.select((state) => state.images));
      ...
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              const TitleAppbar(title: 'Da a conocer tu espacio'),
              Container(
                padding:
              ...
              if (imageFileList.isNotEmpty) ...[
                // If the list is not empty, then show the images

                ContainerImages(
                  // This is the main image, the one that is bigger
                ...
                GridView.count(
                  ...
                  shrinkWrap:
                      true, // If true, the extent of the scroll view in the scrollDirection is determined by the contents being viewed
                  physics:
                      const NeverScrollableScrollPhysics(), // Disables scrolling in the GridView
                  children: imageFileList.skip(1).map((e) {
                    return Padding(
                      padding: const EdgeInsets.all(
                          8.0), // Add some padding around each image
                      child: ContainerImages(
                        ...
                        deleteImage: () {
                          int index =
                              ref.read(imagesHouseProvider).images.indexOf(e);
